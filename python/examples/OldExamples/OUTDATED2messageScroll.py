...
from constants import alphabet, space, period, letterSpace
...
if __name__ == '__main__':
    ...
    strip.begin()
    message = "Hey Dad I think it works  ".lower()
    ledArr = [[], [], [], [], []]
    loc = 0
    ...
    for i in range(len(message)):
        ...
        if message[i].isalpha():
            loc = copyLetter(loc, ledArr, alphabet[ord(message[i])-ord('a')])
            for d in range(5):
                ledArr[d].append(0)
            loc = copyLetter(loc, ledArr, letterSpace)
        elif ord(message[i]) == ord('.'):
            loc = copyLetter(loc, ledArr, period)
        else:
            loc += 3        #it's a space or an unknown character
    for a in range(5):
        for b in range(3):
            ledArr[a].append(0)
